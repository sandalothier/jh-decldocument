package com.brain.decldocument.web.rest;

import com.brain.decldocument.config.KafkaProperties;
...
class DecldocumentKafkaResourceIT {
    ...
    void setup() {
        ...
        DecldocumentKafkaResource kafkaResource = new DecldocumentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/decldocument-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/decldocument-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
